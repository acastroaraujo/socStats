...
Here, we want to choose the parameters $\alpha$, $\beta_1$, and $\beta_2$ that maximize the *joint probability* of the observed data. Strictly speaking, we want choose the parameters that maximize the following expression:
...
We will calculate the sum of log-likelihoods ( $ll$ ) for each row in this grid, and then choose the combination that produces the maximum value in @eq-logLik.
...
Now we apply the function to each row of the grid. There are more than a million rows, so it might take a few minutes!
...
Hopefully you can see why this form of estimation looks a little bit like mountain climbing.
...
2.  Specify the `data` argument. An error will show up if the variables you specified in the formula don't exist in the data.
3.  Specify the `family` argument. For a logistic regression this will always be "binomial."
...
-   The *deviance* is just the log-likelihood multiplied by -2.
    ...
    You can look at the log-likelihood by using the `logLik()` function. It should roughly correspond to the `ll` value we calculated earlier doing the grid search.
...
The [broom](https://broom.tidymodels.org/) package, with it's associated three functions.
...
The [modelsummary](https://modelsummary.com/index.html) package, which allows for the creation of publication ready tables.

```{r}
modelsummary::msummary(list("Model 1" = mod))
```
...
```{r}
msummary(list(mod1, mod2, mod3, mod4), gof_map = "none")
```
:::
